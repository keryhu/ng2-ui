...
export class SearchUserDetailedComponent implements OnInit{
  ...
  refreshUsers(isClickStart?:boolean){
    ...
    this.http.get(this.url, this.request.getAuthOptions(p))
      .map(res=>res.json())
      .subscribe(
        e=>{
          if(e){
  ...
  // 点击 排序，促发的事件  参数boolean，如果为true，代表是从 页面，因为含有sort，
  // 引起的 排序，否则是用户点击'排序'产生的排序
  sortUsers(init?:boolean){
    const p = new URLSearchParams();
    this.setSearchParam(p);
